  ...
  return (
    <div>
     ...
     <div className="flex justify-center items-center my-12 text-center">
          <Tabs>
            <TabList>
              <Tab ><h1 className="font-exo font-bold">All Meals</h1></Tab>
              <Tab><h1 className="font-exo font-bold">Break Fast</h1></Tab>
              <Tab><h1 className="font-exo font-bold">Lunch</h1></Tab>
              <Tab><h1 className="font-exo font-bold">Dinner</h1></Tab>
            </TabList>

           {
            isLoading ? <LoadingSpinner smallHeight={true}/> :
            <TabPanel>
            ...
            <TabPanel>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mt-12">
              {
               allMeals.filter(meal=> meal.category === "breakfast").map( meal => (
               <MealCard key={meal._id}
               meal={meal}
               />))
               }
            ...
            <TabPanel>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mt-12">
              {
               allMeals.filter(meal=> meal.category === "lunch").map( meal => (
               <MealCard key={meal._id}
               meal={meal}
               />))
               }
            ...
            <TabPanel>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mt-12">
              {
               allMeals.filter(meal=> meal.category === "dinner").map( meal => (
               <MealCard key={meal._id}
               meal={meal}
               />))
               }
